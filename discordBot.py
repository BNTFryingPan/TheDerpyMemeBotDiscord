    ...
    while not client.is_closed:
        ...
        for server in client.servers:
            if server.owner.game.type == 1:
                await chat(errorChannel, 'Owner of ' + str(server) + ' is now live on Twitch')
        time.sleep(100)
    ...
    try:
        ...
        if command == '!off': #allows me to turn the bot on and off
            if message.author.id == "304316646946897920":
                if botIsOn:
                    await chat(message.channel, 'Turning off bot...')
                    botIsOn = False
                else:
                    await chat(message.channel, 'Bot is already off!')
            else:
                await chat(message.channel, 'This isn\'t the command you are looking for...')
                
        elif command == '!on': #allows me to turn the bot on and off
            if message.author.id == "304316646946897920":
                if not botIsOn:
                    await chat(message.channel, 'Turning on bot...')
                    botIsOn = True
                else:
                    await chat(message.channel, 'Bot is already on!')
            else:
                await chat(message.channel, 'This isn\'t the command you are looking for...')
            ...
            if message.author.id == "304316646946897920":
                ...
                try:
                    ...
                    await chat(message.channel, tb)
                else:
                    await chat(message.channel, 'No exception occured in running this command!')
            ...
            if message.author.id == "304316646946897920":
                ...
                await chat(message.channel, 'Status is now: ' + message.content.split(' ', 1)[1])
            ...
            if command == '!myid': # puts the users ID in chat. This is not a private thing that nobody should know, you can see anyones ID if you are in devolper mode
                await chat(message.channel, '{0.author.mention}, your unique ID is {0.author.id}.'.format(message))
                ...
                try:
                    ...
                    await chat(message.channel, 'Did you supply the id of the video to listen to?')
                    return
                if arguments[1].lower() in ['current', 'title']:
                    await chat(message.channel, '[' + player.duration + '] ' + player.title)
                elif arguments[1].lower() in ['desc', 'description']:
                    await chat(message.channel, player.description)
                else:
                    try:
                        ...
                        if player.likes > player.dislikes and player.duration <= 600:
                            await chat(message.channel, 'Playing ' + player.title + ' in ' + str(message.author.voice_channel) + '...')
                            player.start()
                            await asyncio.sleep(int(player.duration + 2))
                            await voice.disconnect()
                        else:
                            await chat(message.channel, 'The video needs to have more likes than dislikes and be less that 10 minutes long.')

                    except discord.errors.InvalidArgument:
                        await chat(message.channel, 'You must me in a voice channel to use this command')
                    except discord.errors.ClientException:
                        await chat(message.channel, 'The bot is already in a voice channel...')
                    except Exception as err:
                        await chat(errorChannel, 'Error Playing music... Server: ' + str(message.server) + ', Error: ' + str(err) + ', Check the python console for more information.')
                        await voice.disconnect()
                        raise
                    
            elif command == '!version': # says bot version in chat
                await chat(message.channel, 'Bot Version: ' + cfg.version)
                
            elif command == '!age':
                try:
                    await chat(message.channel, arguments[1] + ' is ' + str(random.randint(0, 100)) + ' years old :Kappa:')
                except IndexError:
                    await chat(message.channel, str(message.author) + ' is ' + str(random.randint(0, 100)) + ' years old :Kappa:')
                    
            elif command == '!hug':
                try:
                    await chat(message.channel, '/me makes ' + str(message.author) + ' hug ' + arguments[1] + '! HUGS! TwitchUnity bleedPurple <3')
                except IndexError:
                    await chat(message.channel, '/me hugs ' + str(message.author) + '! HUGS!!!')
                    ...
                    if message.content.count(space) >= 2:
                        try:
                            ...
                            if commandadd in globalCommands:
                                await chat(message.channel, 'You cant add that command, as it is already a global TDMB command')
                            else:
                                try:
                                    ...
                                    await chat(message.channel, 'The command, ' + commandadd + ' has been added')
                                    with open("commands.json", "w") as commandsDatabase:
                                        json.dump(commandsList, commandsDatabase)
                        except IndexError:
                            await chat(message.channel, '[ERROR] Did you use the command the right way? !addcom <commandTrigger> <output>')
                            if debug == true:
                                await chat(str(sys.exc_info()[0]))
                            
                else:
                    await chat(message.channel, 'You dont have permission to use that command!')
                ...
                if message.author.id in customCommandManagers or message.author == message.server.owner:
                    if message.content.count(space) == 1:
                        try:
                            ...
                            await chat(message.channel, 'Did you use the command the right way?')
                            blab = 1
                            if debug == true:
                                await chat(message.channel, str(sys.exc_info()[0]))

                        if blab == 0:
                            if commanddel in globalCommands:
                                await chat(message.channel, 'You can\'t delete a global command')
                            else:
                                try:
                                    print(str(commandsList[str(message.server.id)][commanddel]))
                                except KeyError:
                                    await chat(message.channel, 'You cannot delete a command that doesn\'t exist')
                                    blab = 1

                                if blab == 0:
                                    del commandsList[str(message.server.id)][commanddel]
                                    await chat(message.channel, 'Command Deleted.')
                                    with open("commands.json","w") as commandsDatabase:
                                        json.dump(commandsList, commandsDatabase)
                else:
                    await chat(message.channel, 'You dont have permission to use that command!')
                    ...
                    if message.content.count(space) >= 2:
                        try:
                            ...
                            if commandedit in globalCommands:
                                await chat(message.channel, 'You can\t edit global commands')
                            else:
                                try:
                                    commandsList[str(message.server.id)][commandedit] = answer
                                    await chat(message.channel, 'The command, ' + commandedit + ' has been edited')
                                    with open("commands.json", "w") as commandsDatabase:
                                        json.dump(commandsList, commandsDatabase)
                                except KeyError:
                                    await chat(message.channel, 'That command doesnt exist.')
                                    
                        except IndexError:
                            blab = 1
                            await chat(message.channel, 'Did you use the command the right way? !editcom <commandTrigger> <output>')
                            if debug == true:
                                await chat(message.channel, str(sys.exc_info()[0]))
                        ...
                        if message.author.id in quoteManagers or message.author == message.server.owner:
                            try:
                                ...
                                try:
                                    ...
                                    await chat(message.channel, 'Quote Added as #' + quoteNumber + '!')
                                    with open('quotes.json', 'w') as quoteDatabase:
                                        json.dump(quotesList, quoteDatabase)
                                        
                            except IndexError:
                                await chat(message.channel, 'Did you provide a quote to add?')
          
                        else:
                            await chat(message.channel, 'You dont have permission to add a quote!')
                        ...
                        if message.author.id in quoteManagers or message.author == message.server.owner:
                            try:
                                ...
                                try:
                                    ...
                                    await chat(message.channel, 'Deleted Quote #' + quotedel)
                                    with open('quotes.json', 'w') as quoteDatabase:
                                        json.dump(quotesList, quoteDatabase)
                                except KeyError:
                                    await chat(message.channel, 'You cant delete a quote that doesnt exist.')
                            except IndexError:
                                await chat(message.channel, 'Did you provide a quote number to delete?')
                        else:
                            await chat(message.channel, 'You do not have permission to delete a quote!')
                    else:
                        try:
                            quoteNumber = str(int(arguments[1]))
                            
                            try:
                                await chat(message.channel, 'Quote #' + quoteNumber + ': ' + quotesList[message.server.id][quoteNumber])
                            except (IndexError, KeyError):
                                await chat(message.channel, 'That quote doesnt exist!')
                        except ValueError:
                            await chat(message.channel, 'Did you enter a quote number?')
                            
                except IndexError:
                    try:
                        randomQuote = random.choice(list(quotesList[message.server.id].keys()))

                        await chat(message.channel, 'Quote #' + str(randomQuote) + ': ' + quotesList[message.server.id][str(randomQuote)])
                    except (IndexError, KeyError):
                        await chat(message.channel, 'There are no quotes yet!')
                        
            else:
                try:
                    for key in commandsList[str(message.server.id)]:
                        if message.content.startswith(key): # NORMAL custom commands
                            await chat(message.channel, str(commandsList[str(message.server.id)].get(key)).format(user = str(message.author.mention), time=message.timestamp.time()).split('.')[0], )
                except KeyError:
                    pass
...
async def on_ready():
    ...
    await chat(hard_coded_channel, 'The Derpy Meme Bot version ' + cfg.version + ' is now running!')
